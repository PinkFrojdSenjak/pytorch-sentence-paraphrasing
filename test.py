...
# %%
t = torch.tensor([float('-inf')  for i in range(10)] )
from torch.nn import functional
functional.softmax(t)


# %%
mask = model.get_tgt_mask(4)
t  = torch.ones(4)
mask = nn.Transformer.generate_square_subsequent_mask(32,32)
temp = tempSrc[0].to(torch.float32)

print(torch.mm(mask, temp.unsqueeze(1)))
...
torch.triu(torch.ones((1, 4, 2)), diagonal=1)

...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        self.embedding = nn.Embedding(100000, dim_model, padding_idx=0)
    ...
    def get_tgt_mask(self, size) -> torch.tensor:
        ...
        mask = mask.float()
        mask = mask.masked_fill(mask == 0, float(-maxint//2)) # Convert zeros to -inf
        mask = mask.masked_fill(mask == 1, float(0.0)) # Convert ones to 0
        return mask'''

    def get_tgt_maska(self, size) -> torch.tensor:
        mask = torch.tril(torch.ones(size, size) == 1) # Lower triangular matrix
        mask = mask.bool()
        return mask
